...
function CreateWorkout({ createWorkout, categories, propExercises }) {
	...
	const deleteExercise = (exerciseToDelete) => {
		const newExercises = exercises.filter((exercise) => exerciseToDelete !== exercise.name);
		...
		<main className="flex column flex-grow">
			...
			<form className="base-form" onSubmit={(e) => onSubmit(e)}>
				...
				<ul className="categories active-exercise-list">
					{exercises.map((exercise) => (
						<li key={exercise.name} className="active-exercise">
							{exercise.name}
							<button type="button" onClick={() => deleteExercise(exercise.name)} className="btn btn-xs btn-secondary btn-delete">X</button>
						</li>
					))}
					<li>
						<button onClick={handleClickOpen} className="btn btn-secondary btn-link" type="button">ADD EXERCISE</button>
					</li>
				</ul>
				...
				<div className="flex column flex-grow justify-end m-4">
					<button type="submit" className="btn btn-primary btn-l self-center">SAVE WORKOUT</button>
				</div>
				<WorkoutAddExerciseDialog exercises={propExercises} categories={categories} open={open} onClose={handleClose} />
			</form>
		</main>
	) : (
			<Spinner />
