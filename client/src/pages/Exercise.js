		...
		<main className="general-main">
			...
			{exercise.superSet ? (
				<div className="flex justify-between">
					<div>
						<span className="bold enlarge">Sets: </span>{exercise.sets}
					</div>
					<div>
						<span className="bold enlarge">Total reps: </span>{exercise.totalReps}
					</div>
				</div>
			) : (
					<div className="flex">
						...
						<div className="ml-4">
							<span className="bold enlarge">Reps: </span>{exercise.reps}
						</div>
					</div>
				)}
			...
			<div className="flex justify-between">
				<Link to={{
					...
					<button type="button" className="btn btn-primary btn-sm">EDIT EXERCISE</button>
				</Link>
...
const mapStateToProps = (state, ownProps) => {
	...
	const [exercise] = state.exercises.exercises.filter((exercise) => exercise.linkName === match.params.exerciseName);
