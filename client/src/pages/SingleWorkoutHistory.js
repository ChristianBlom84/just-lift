...
function SingleWorkoutHistory({ workoutHistory, workouts, singleWorkoutHistory, loading, match }) {
	console.log(singleWorkoutHistory);
	return workouts !== null && workoutHistory !== null && singleWorkoutHistory !== null && !loading ? (
		<main className="general-main">
			<WorkoutHistoryList workouts={workouts} workoutHistory={singleWorkoutHistory} match={match} />
		</main >
	) : (
			<Spinner />
		)
...
const mapStateToProps = (state, ownProps) => {
	...
	singleWorkoutHistory.finishedWorkouts = state.workouts.workoutHistory.finishedWorkouts.filter((historyEntry) => historyEntry.linkName === ownProps.match.params.workoutName);
